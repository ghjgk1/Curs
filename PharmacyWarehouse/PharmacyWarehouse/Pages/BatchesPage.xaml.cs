...
public partial class BatchesPage : Page
{
    ...
    private void UpdateSelectedBatchInfo()
    {
        ...
        {
            // Проверяем, есть ли выбранная партия в отфильтрованном списке
    ...
    private void ClearBatchDetails()
    {
        ...
        if (dgBatchMovements != null)
        {
            dgBatchMovements.ItemsSource = null;
        }
    }
    ...
    private void UpdateBatchDetails(Batch batch)
    {
        try
        {
            ...
            if (fullBatch != null)
            {
                txtSelectedBatch.Text = $"Партия: {fullBatch.BatchNumber}";
                ...
                UpdateBatchMovements(fullBatch);
            }
            else
            {
                ClearBatchDetails();
    ...
    private void UpdateBatchMovements(Batch batch)
    {
        try
        {
            // Временная заглушка
        ...
        {
            Console.WriteLine($"Ошибка при загрузке движений: {ex.Message}");
        }
    }
    ...
    {
        var dialog = new AddEditBatchWindow();
    ...
    private void EditBatch_Click(object sender, RoutedEventArgs e)
    {
        if (SelectedBatch == null)
        {
            MessageBox.Show("Выберите партию для редактирования", "Информация",
                MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }

        var dialog = new AddEditBatchWindow(SelectedBatch);
        if (dialog.ShowDialog() == true)
        {
            LoadData();
        }
    ...
    private void DeleteBatch_Click(object sender, RoutedEventArgs e)
    {
        if (SelectedBatch == null)
        {
            MessageBox.Show("Выберите партию для удаления", "Информация",
                MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }

        var result = MessageBox.Show($"Удалить партию '{SelectedBatch.BatchNumber}'?\n" +
                                    $"Товар: {SelectedBatch.Product?.Name}\n" +
                                    $"Остаток: {SelectedBatch.Quantity}",
    ...
    private void WriteOffBatch_Click(object sender, RoutedEventArgs e)
    {
        ...
        var dialog = new WriteOffBatchWindow(SelectedBatch);
        if (dialog.ShowDialog() == true)
        {
            // Обновляем данные
            LoadData();
            ...
            MessageBox.Show("Списание успешно выполнено и сохранено в системе",
    ...
    private void ResetFilters_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            SearchBatch.BatchNumber = string.Empty;
